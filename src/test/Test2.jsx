...
export default function WaterReflectionText({
  text = "we don't\nmake ads.",
  rippleDetail = 18,
  rippleSpeed = 2,
  distortion = 6,
  ...
  // scroll-driven "pop out of the water" progress, 0 = submerged/clipped,
  // 1 = fully risen, scaled up and out of the water
  const [popRaw, setPopRaw] = useState(0);
  ...
  useEffect(() => {
    ...
    function onWheel(e) {
      setPopRaw((prev) => {
        const next = Math.max(0, Math.min(popScrollRange, prev + e.deltaY));
        // splash whenever a scroll gesture pushes the text fully out of
        // (or fully back into) the water
        if ((prev < popScrollRange && next >= popScrollRange) || (prev > 0 && next <= 0)) {
          spawnRipple();
        }
        return next;
      });
  ...
  const pop = popRaw / popScrollRange;
  ...
  const popRise = pop * horizonY * 0.55;
          ...
          >
            <div
              style={{
                ...
                transform: `translate(-50%, calc(-${halfLineHeight}px - ${popRise}px)) scale(${popScale}) perspective(700px) rotateX(${(1 - pop) * 9}deg)`,
                transition:
